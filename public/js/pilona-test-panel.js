// pilona-test-panel.js - Panel de pruebas en tiempo real para pilonas
...
function initTestPanel() {
  console.log('Inicializando panel de pruebas de pilona...');
  ...
  console.log('Panel de pruebas inicializado');
}
...
async function probarConexion() {
  ...
  try {
    ...
    if (response.ok && result.success) {
      ...
      // Verificar si el modo pruebas ya está activo y mostrar panel
      const modoPruebasCheckbox = document.getElementById('modo-pruebas-activo');
      if (modoPruebasCheckbox && modoPruebasCheckbox.checked) {
        panelCoils.style.display = 'block';
        console.log('Panel de pruebas mostrado - modo pruebas ya estaba activo');
      } else {
      ...
      // Iniciar lectura periódica de estados si auto-refresh está habilitado
      const autoRefreshCheckbox = document.getElementById('test-auto-refresh');
...
function actualizarTablaPruebas(datosConexion) {
  ...
  if (tipoDispositivo === 'LOGO') {
    // Para LOGO, mostrar las direcciones configuradas
    if (datosConexion.logoConfig) {
      document.getElementById('test-addr-subir').textContent = datosConexion.logoConfig.subir?.elemento || '-';
      document.getElementById('test-addr-bajar').textContent = datosConexion.logoConfig.bajar?.elemento || '-';
      document.getElementById('test-addr-estado').textContent = datosConexion.logoConfig.estado?.elemento || '-';
      document.getElementById('test-addr-bloqueo').textContent = datosConexion.logoConfig.bloqueo?.elemento || '-';
      document.getElementById('test-addr-puntual').textContent = datosConexion.logoConfig.puntual?.elemento || '-';
    }
  } else {
...
function habilitarBotonesPrueba(habilitar) {
  ...
  // Habilitar/deshabilitar checkbox de auto-refresh
  const autoRefreshCheckbox = document.getElementById('test-auto-refresh');
  if (autoRefreshCheckbox) {
    autoRefreshCheckbox.disabled = !habilitar;
  }
  
  // Habilitar/deshabilitar checkbox de modo pruebas
  const modoPruebasCheckbox = document.getElementById('modo-pruebas-activo');
  if (modoPruebasCheckbox) {
...
async function escribirCoil(coil) {
  ...
  try {
    ...
    if (response.ok && result.success) {
      ...
      // Revertir a 0 después de 1 segundo
      setTimeout(async () => {
...
function iniciarLecturaPeriodica() {
  ...
  testPanelState.intervalId = setInterval(async () => {
    ...
    // Verificar si el auto-refresh sigue activo
    const autoRefreshCheckbox = document.getElementById('test-auto-refresh');
    if (!autoRefreshCheckbox || !autoRefreshCheckbox.checked) {
      clearInterval(testPanelState.intervalId);
      testPanelState.intervalId = null;
...
function resetearPanelPruebas() {
  ...
  if (panelCoils) panelCoils.style.display = 'none';
...
// Función helper para obtener direcciones LOGO mapeadas
function obtenerDireccionesLOGO(logoConfig) {
  const direcciones = {};
  
  // Mapear cada función a su dirección Modbus
  ['subir', 'bajar', 'estado', 'bloqueo', 'puntual'].forEach(funcion => {
    if (logoConfig[funcion] && logoConfig[funcion].elemento) {
      // Aquí se debería usar la función de mapeo real del servidor
      // Por ahora, usamos un mapeo simplificado
      const elemento = logoConfig[funcion].elemento;
      let direccion = 0;
      
      if (elemento.startsWith('I')) {
        direccion = 10000 + parseInt(elemento.substring(1)) - 1;
      } else if (elemento.startsWith('Q')) {
        direccion = parseInt(elemento.substring(1)) - 1;
      } else if (elemento.startsWith('M')) {
        direccion = 8192 + parseInt(elemento.substring(1)) - 1;
      }
      
      direcciones[funcion] = direccion;
    }
  });
...
function actualizarModoPruebas(activo) {
  ...
  if (activo) {
    ...
    // Verificar que hay conexión antes de mostrar panel
    if (testPanelState.conexionActiva && panelPruebasCoils) {
      panelPruebasCoils.style.display = 'block';
      console.log('Panel de pruebas mostrado - conexión activa');
    } else if (!testPanelState.conexionActiva) {
      // Si no hay conexión, advertir al usuario y revertir checkbox
    ...
    if (panelPruebasCoils) {
      ...
      console.log('Panel de pruebas ocultado');
    }
  }
}
...
console.log('Módulo pilona-test-panel.js cargado');
